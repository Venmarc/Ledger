...
- Right: "View Demo" button (primary) + "GitHub" link (icon + text).
- Sticky. Transparent on scroll-top, solid dark background on scroll.
- No hamburger menu. No nav links. Two actions only.
...
- Rate source note: "Rates from [source name]. Last updated [timestamp]."
- Error state: "Rates unavailable. Check your connection."
- Widget fetches rates on page load and caches for the session. Does not refetch on every input change — rates are calculated client-side after initial fetch.
