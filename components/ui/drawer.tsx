...
function DrawerOverlay({
  ...
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn(
        // Above FAB (z-50) so sheet chrome owns the stack
        "fixed inset-0 z-[100] bg-black/60 supports-backdrop-filter:backdrop-blur-[4px] data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className
      )}
      {...props}
    />
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      <DrawerPrimitive.Content
        ...
        className={cn(
          "group/drawer-content fixed z-[110] flex flex-col bg-bg-elevated text-sm text-text-primary outline-none focus:outline-none focus-visible:outline-none",
          // Intrinsic height from children — do not use h-auto + flex-1 children (collapses to blank overlay)
          "data-[vaul-drawer-direction=bottom]:inset-x-0 data-[vaul-drawer-direction=bottom]:bottom-0 data-[vaul-drawer-direction=bottom]:mt-16 data-[vaul-drawer-direction=bottom]:max-h-[92vh] data-[vaul-drawer-direction=bottom]:rounded-t-[14px] data-[vaul-drawer-direction=bottom]:border-t data-[vaul-drawer-direction=bottom]:border-border",
        ...
        {...props}
      >
        {/* UIUX §6.4: always-visible drag handle for bottom sheets */}
        <div
          className="mx-auto mt-3 mb-1 h-1 w-8 shrink-0 rounded-full bg-border-strong"
          aria-hidden
        />
        {children}
      </DrawerPrimitive.Content>
